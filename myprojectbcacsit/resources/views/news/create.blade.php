@extends('layouts.app')
@section('content')
<h2 class="text-4xl text-blue-600 border-b-4 border-blue-300">Add New News</h2>
...
<form action="{{route('news.store')}}" method="POST" class="py-5" enctype="multipart/form-data">
    ...
    <input type="text" class="w-full rounded-lg mt-5" placeholder="Title" name="title" value="{{old('title')}}">
    @error('title')
        <p class="text-red-600 text-sm">* {{$message}}</p>
    @enderror
